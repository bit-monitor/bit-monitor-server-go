...
func (c *customLogger) Printf(message string, data ...interface{}) {
	if global.WM_CONFIG.Mysql.LogZap != "" {
		switch len(data) {
		case 0:
			global.WM_LOG.Info(message)
		case 1:
			global.WM_LOG.Info("gorm", zap.Any("src", data[0]))
		case 2:
			global.WM_LOG.Info("gorm", zap.Any("src", data[0]), zap.Any("duration", data[1]))
		case 3:
